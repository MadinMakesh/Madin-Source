...
sub Run {
    ...
    if ( $Self->{ArticleFilterActive} ) {
        ...
        my @testArticleTypeIDs = $Self->{ArticleFilter}->{ArticleTypeID};
        my @filters123 = (1,2,5,6,7,8,9,10,11);
        push @testArticleTypeIDs, @filters123;
        $Self->{ArticleFilter}->{ArticleTypeID} = \@testArticleTypeIDs;
    ...
    # generate output
	open FH,(">>for_subject.txt");
	print FH "iofhodfh:::\t$Ticket{TicketNumber}\t$Ticket{TicketID}";
	close FH;
