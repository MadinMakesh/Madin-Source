...
#jugni custom code. Added TicketSource in below key array
    for my $Key (
        ...
        StandardTemplateID FromChatID TicketSource
        )
        )
    {
...
# ---
    ...
    # create new ticket and article
        ...
        # get from and customer id if customer user is given
			...
			#########################################code for Customer User Exist########33
				my $full_customer_user;
			#########################################code for Customer User Exist########33
            ...
            for my $KeyCustomerUser ( sort keys %CustomerUserList ) {
				...
				#########################################code for Customer User Exist########33
				 $full_customer_user="$CustomerUserList{$KeyCustomerUser}";
				 #########################################code for Customer User Exist########33
            }
			...
			#########################################code for Customer User Exist########33
                ...
                # $FromExternalCustomer{Email} = $ExternalCustomerUserData{UserEmail};
                $FromExternalCustomer{Email} = $full_customer_user;
        ...
        if (%Error) {
            ...
            $Output .= $LayoutObject->Footer();
			#########################################code for Customer User Exist
			$Output=~s/\&quot\;//igs;$Output=~s/\&lt\;//igs;$Output=~s/\&gt\;//igs;
			#########################################code for Customer User Exist
            return $Output;
        }
